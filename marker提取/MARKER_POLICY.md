...
本文件是当前 Marker 提取、复核和入库的唯一范围约束。若 Gemini 输出、历史记录或既有结果与本文件冲突，以本文件为准。
...
- `表单/our_markers.xlsx` 是当前正式 Marker 总表；其他历史文件不得自动覆盖或批量合并到总表。
- 新结果必须先由 Gemini 逐篇提取，再由 Codex 对照原始论文与补充材料复查；只有通过上述证据要求的记录才能入表。
- 旧自动提取、批量审计和恢复管线已经停用。清理前状态保存在 Git 分支 `feat/0903` 的提交 `9698859`，仅用于必要时追溯。
